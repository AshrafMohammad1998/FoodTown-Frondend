...
function PartnerOrders() {
  ...
  const fetchRestaurantOrders = async() => {
    try {
      const response = await axios.get(`${configVariables.ipAddress}/orders/getPartnerOrders/${restaurantData?._id}`, {
        headers: {
    ...
    <div className='px-0 sm:px-10 pb-8 mb-12 sm:mb-0'>
      <ul>
        {orders.map(eachOrder => (
          <PartnerOrderItems key={eachOrder._id} order = {eachOrder} fetchData = {fetchRestaurantOrders}/>
        )) }
      </ul>
    </div>
  )
