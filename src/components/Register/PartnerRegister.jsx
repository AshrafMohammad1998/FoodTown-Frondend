      ...
      <div className="flex items-center justify-center min-h-screen pb-24 pt-14">
        <div className="relative flex flex-col p-3 w-96 border border-slate-500 rounded-2xl shadow-2xl mt-36 md:w-3/5">
          ...
          {showForm === "owner" && (
           ...
           {showForm === "restaurant" && <RestaurantDetails onBack={handleBack} onNext={handleRestaurantNext}/>}
          {showForm === "otp" &&<PartnerOTP onBack={handleBack} email={email} name={name} mobile={mobile} password={password}/>}
        </div>
      </div>
    </div>
  );
