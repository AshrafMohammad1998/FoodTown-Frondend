...
function Restaurant() {
  ...
  return (
    <div className="px-2 pb-8 mb-12 sm:mb-0 ">
      <h1 className="sticky top-12 z-20 m-1 mb-4 text-xl font-bold underline tracking-wide w-full text-center bg-white dark:bg-slate-800">
      ...
      <ul className="bg-white dark:bg-slate-800 w-[90%] md:w-4/5 m-auto divide-y divide-slate-400">
        {isLoading
          ? new Array(5).fill("").map((e, index) => <DishItemSkeleton key={index} />)
          : dishes.map((eachDish) => (
              <DishItem
                key={eachDish._id}
                dish={eachDish}
