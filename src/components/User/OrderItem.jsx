...
function OrderItem(props) {
  ...
  return (
    <div className="border border-slate-300 rounded-2xl shadow-xl w-[90%] md:w-3/4 m-auto mt-4">
      <div className="flex justify-between w-full bg-slate-100 rounded-t-2xl text-black dark:text-slate-500 p-2 sm:p-4">
        ...
        <div className="flex-grow flex justify-between m-0">
          <div className="">
            <h3 className="font-bold text-md sm:text-xl">{restaurantDetails.name}</h3>
            <p className="text-xs sm:text-lg">{restaurantDetails.address}</p>
            <p className="text-xs sm:text-sm text-red-500">Opens at {fromTime12Hour(restaurantDetails.fromTime)}</p>
          </div>
          <div className="flex flex-col justify-between ml-">
            <p className="bg-slate-400 px-3 py-2 rounded-lg text-xs sm:text-md text-black">{order.status}</p>
            <button
          ...
          <ul className="w-full p-2 sm:p-4">
            {order.dishes.map((eachDish) => (
              <li  key={useId()} className="flex w-full ">
                <p className="text-slate-400 font-bold">{eachDish.quantity} x </p>
                <p className="ml-2 truncate text-sm sm:text-lg">{eachDish.dishDetails.dishName}</p>
              </li>
            ))}
          </ul>
        )}
